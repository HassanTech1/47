...
export async function action({request, context}) {
  ...
  return data({cart: cartResult, errors, analytics: {cartId: result.cart.id}}, {status, headers});
}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-1">
              <div className="border-2 border-black p-6">
                ...
                {checkoutError && <p className="mt-4 text-xs text-red-600 leading-relaxed">{checkoutError}</p>}
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <div className="flex gap-6 border-b border-gray-100 pb-8">
      <div className="w-24 h-32 bg-gray-50 flex-shrink-0 overflow-hidden">
        {item.image ? (
          <img src={item.image} alt={item.nameEn ?? item.name ?? 'Product'} className="w-full h-full object-contain" />
        ) : (
      ...
      <div className="flex-1">
        <div className="flex justify-between">
          ...
          <button onClick={() => onRemove(item.id, item.size)} aria-label="Remove item">
